...
public class PanelUnidade {
	...
	private Color cor;
	private OponentesDerrotadosPanel oponentesDerrotadosPanel;
	 ...
	 *            que representa
	 */
	public PanelUnidade(Color cor, Unidade unidade, OponentesDerrotadosPanel oponentesDerrotadosPanel) {

		this.cor = cor;
		this.unidade = unidade;
		this.oponentesDerrotadosPanel = oponentesDerrotadosPanel;
	 ...
	 * 
	 * @param total
	 *            se a descrição de "OD" deve ser "OD Total"
	 */
	...
	public void createPanelDados() {
		...
		quantidade = new IntegerFormattedTextField(9, Integer.MAX_VALUE) {
			...
			public void go() {
				...
				oponentesDerrotadosPanel.total.setTotal();
	...
	/**
	 * Multiplica a quantidade pelo OD individual, dependendo de qual
	 * RadioButton está escolhido no OponentesDerrotadosPanel
	 */
	protected void changeOD() {

		BigDecimal quantia = quantidade.getValue();

		if (oponentesDerrotadosPanel.buttonAtaque.isSelected())
			lblOD.setText(quantidade.numberFormat.format(oponentesDerrotadosPanel.getODA(unidade)
					.multiply(quantia)));

		else if (oponentesDerrotadosPanel.buttonDefesa.isSelected())
			lblOD.setText(quantidade.numberFormat.format(oponentesDerrotadosPanel.getODD(unidade)
					.multiply(quantia)));
	}
